...
export default function Deposit() {
  ...
  const refetch = async () => {
    try {
      ...
      /**
       * `direction`, not `dir`, and `offset`, not `page` — the service strips
       * query keys it does not declare, so the old names filtered nothing and
       * every page returned the first 25 rows.
       */
      const { data } = await apiFetchPage<Transfer>(
        uid ? buildPath(ENDPOINTS.staff.transfersFor, { staffId: uid }) : ENDPOINTS.staff.transfers,
        { query: { direction: "deposit", limit: pg.size, offset: (pg.page - 1) * pg.size } }
      );
      setRows(data);
  ...
  /* ---------------- filter in memory ----------------------------------- */
  // Agent Users tab: only show transfers from a staff/agent to a player (user).
  const view = useMemo(() => rows.filter(r => {
    if (r.from?.type !== "staff" || r.to?.type !== "user") return false;
    ...
    <Box sx={{ p: 3, maxWidth: 1400, mx: "auto" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Box sx={{ p: 1, bgcolor: "rgba(14,204,104,0.15)", borderRadius: 2, display: "flex" }}>
            <ArrowUpCircle size={22} color="#0ECC68" />
          </Box>
          <Box>
          ...
          <Table size="small">
            ...
            <TableBody>
              {load ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} sx={{ textAlign: "center", py: 5, borderBottom: "none" }}>
                    <ArrowUpCircle size={36} color="#1E2D55" />
                    <Typography sx={{ color: "#8384A5", mt: 1 }}>No deposits found.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                view.map((r) => (
