...
const FancyResultNotShowingFromApi: React.FC = () => {
  ...
  const fetchData = async (pageToFetch: number) => {
    ...
    try {
      ...
      let url = `${BASE_URL}${ENDPOINTS.sportsResults.fancy}?page=${pageToFetch}&limit=${limit}`;
      if (searchTerm) url += `&search=${encodeURIComponent(searchTerm)}`;

      const headers: Record<string, string> = {
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
        // `x-staff-id` is stripped at the gateway — scope comes from the token.
      };
  ...
  return (
    <div className="p-6">
      <div className="bg-[#0C0D1D] rounded-lg shadow-md p-6 mb-6">
        <h1 className="text-2xl font-bold mb-6">Fancy Api Report</h1>

        {/* Filters */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label htmlFor="search-input" className="block text-sm font-medium text-[#878AA2] mb-1">
              Search
            </label>
            <input
              id="search-input"
          ...
          <div className="flex items-end justify-end">
            <button
              ...
              className="px-4 py-2 bg-[#0E1831] rounded border hover:bg-[#162140] disabled:opacity-50"
        ...
        {loading && rows.length === 0 && (
          <div className="text-center py-10">
            <p className="text-[#878AA2]">Loading fancy results...</p>
          </div>
        )}

        {!loading && rows.length === 0 && (
          <div className="text-center py-10">
            <p className="text-[#878AA2]">No fancy results found</p>
          </div>
        )}
        ...
        {rows.length > 0 && (
          <div className="overflow-x-auto">
            <table className="w-full bg-[#0C0D1D] shadow-md rounded-lg overflow-hidden">
              <thead className="bg-[#0E1831] border-b">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#878AA2] uppercase tracking-wider">ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#878AA2] uppercase tracking-wider">Fancy Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#878AA2] uppercase tracking-wider">Event ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#878AA2] uppercase tracking-wider">Match ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#878AA2] uppercase tracking-wider">Last Fetch Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1E2D55]">
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-[#0E1831] transition-colors">
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-center mt-6">
            <button
              ...
              className="px-4 py-2 mr-4 bg-[#0E1831] rounded border hover:bg-[#162140] disabled:opacity-50"
              aria-label="Previous page"
            >
              Previous
            </button>

            <div className="text-center px-4 py-2 bg-[#0E1831] rounded border min-w-[80px]">
            ...
            <button
              ...
              className="px-4 py-2 ml-4 bg-[#0E1831] rounded border hover:bg-[#162140] disabled:opacity-50"
