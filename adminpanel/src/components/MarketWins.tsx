...
const MarketHistory: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `${BASE_URL}${ENDPOINTS.sportsResults.markets}?page=${pagination.currentPage}&limit=${pagination.limit}`;
    ...
    <Box sx={{ p: 3, maxWidth: 1400, mx: "auto" }}>
      ...
      <Paper sx={{ ...cardSx, p: 2, mb: 2 }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ sm: "center" }}>
          <TextField
            size="small"
            placeholder="Search match, event, winner…"
            value={searchTerm}
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={thSx}>Winner</TableCell>
                <TableCell sx={{ ...thSx, textAlign: "right" }}>Team1 Ex</TableCell>
                <TableCell sx={{ ...thSx, textAlign: "right" }}>Team2 Ex</TableCell>
                <TableCell sx={{ ...thSx, textAlign: "right" }}>Draw Ex</TableCell>
                <TableCell sx={{ ...thSx, textAlign: "right" }}>Total Bets</TableCell>
              ...
              {loading && rows.length === 0 ? (
                  ...
                  <TableRow key={i}>
                    {[...Array(11)].map((_, j) => (
                      <TableCell key={j} sx={tdSx}>
                        <Skeleton variant="text" width={j === 0 ? 140 : 70} sx={{ bgcolor: "#162140" }} />
                      </TableCell>
                    ))}
                  </TableRow>
                ))
                ...
                <TableRow>
                  <TableCell colSpan={11} sx={{ textAlign: "center", py: 6, borderBottom: "none" }}>
                    ...
                    <Typography sx={{ color: "#8384A5", mt: 1 }}>No market results found.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                rows.map((r) => (
                  ...
                  >
                    ...
                    <TableCell sx={{ ...tdSx, textAlign: "right", fontFamily: "monospace" }}>
                      {formatNumber(r.team1ex)}
                    </TableCell>
                    <TableCell sx={{ ...tdSx, textAlign: "right", fontFamily: "monospace" }}>
                      {formatNumber(r.team2ex)}
                    </TableCell>
                    <TableCell sx={{ ...tdSx, textAlign: "right", fontFamily: "monospace" }}>
                      {formatNumber(r.drawex)}
                    </TableCell>
                    <TableCell sx={{ ...tdSx, textAlign: "right" }}>
                      <Chip
                        label={formatNumber(r.totalbets)}
                        size="small"
                        sx={{ bgcolor: "rgba(136,108,255,0.12)", color: "#A08FFF", fontWeight: 600, fontSize: "0.72rem" }}
                      />
                    </TableCell>
                    <TableCell sx={{ ...tdSx, textAlign: "right" }}>
                      <Chip
                        label={`₹ ${formatNumber(r.payout)}`}
                        size="small"
                        sx={{
